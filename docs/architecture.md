...
| Docker Compose | Runs Postgres and Redis | A single command gives a clean, reproducible database. The API and frontend run on the host for fast reloads, and Ollama and training run natively for direct GPU access. | Everything in containers. GPU passthrough on Windows is one more thing that can break. |
